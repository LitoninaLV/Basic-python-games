...
#from tictactoe_sample.game import play
...
class Player():
    ...
    # we eant all players to get their next move
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if player == max_player:
            ...
            best = {'position': None, 'score': math.inf} # each score should minimize (be lower)
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves()) # randomly choose a square to go to
        else:
