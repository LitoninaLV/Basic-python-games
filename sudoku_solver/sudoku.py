...
def solve_sudoku(puzzle):
    ...
    # Step 1.1: if there's no spaces left, then we're done because we only allowed valid inputs
    if row is None:
        return True

    # Step 2: if there is a place to put the number, then make a guess between 1 and 9
    for guess in range(1,10): # range(1, 10) is 1, 2, 3, ... 9
        ...
        # Step 5: if not valid OR our guess is wrong and does not solve the puzzle, then we need to 
        # backtrack and try a new number
        puzzle[row][col] = -1 # reset  

    # step 6: if none of the numbers that we try work, then this puzzle
    # is unsolvable
    return False
